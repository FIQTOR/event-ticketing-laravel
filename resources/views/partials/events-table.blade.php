<span>Total: <span class="font-medium">{{ $eventsCount }}</span></span>

<form action="{{ route('events.deleteSelected') }}" method="POST" class="w-full">
    @csrf
    @method('DELETE')
    ...
    <table class="w-full">
        <thead>
            <tr class="text-neutral-200 bg-neutral-900">
                ...
                <th></th>
                <th class="px-4">
                    <button onclick="HandleOrderBy(event, `{{ $orderBy === 'desc' ? ' asc' : 'desc' }}`, 'title')"
        ...
        <tbody>
            ...
            @foreach ($events as $event)
                <tr class="@if ($i % 2 == 0) bg-yellow-100 @endif">
                    <td class="text-center rounded-l-3xl">
                    ...
                    <td class="px-4">{{ $event->description }}</td>
                    <td class="px-4">Rp{{ number_format($event->price, 0, '.', '.') }}</td>
                    <td class="px-4">{{ $event->updated_at }}</td>
                    <td class="px-4 rounded-r-3xl">{{ $event->created_at }}</td>
                </tr>
                @php
                    $i++;
                @endphp
