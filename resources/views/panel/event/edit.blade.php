...
@section('container')
    <main class="py-24 px-7 md:px-14">
        <form id="edit-form" action="{{ route('events.update', $event->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <aside class="flex flex-col gap-4 md:w-1/2">
                <div class="flex flex-col gap-4">
                    <div class="flex items-center justify-center">
                        <img id="preview_thumbnail"
                            src="{{ Str::startsWith($event->thumbnail, 'https://') ? $event->thumbnail : asset('storage/' . $event->thumbnail) }}"
            ...
            <div class="flex flex-col gap-4 md:w-full">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex flex-col">
                        <input type="text" name="title" id="title" required value="{{ $event->title }}"
                            placeholder="example: Vanilla Late"
                            class="px-4 py-2 rounded-full outline outline-1 focus:outline-amber-900">
                        @error('title')
                            <span class="text-red-400">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex flex-col">
                        <input type="text" name="organizer" id="organizer" required value="{{ $event->organizer }}"
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="flex flex-col">
                        <input type="text" name="address" id="address" required value="{{ $event->address }}"
                            placeholder="example: Ekspectanica"
                            class="px-4 py-2 rounded-full outline outline-1 focus:outline-neutral-900">
                        @error('address')
                            <span class="text-red-400">{{ $message }}</span>
                ...
                <div class="flex flex-col gap-2 w-full">
                    ...
                    <input type="number" name="price" id="price"
                        class="px-4 py-2 rounded-full outline outline-1 focus:outline-amber-900" value={{ $event->price }}
                ...
                <div class="flex gap-4">
                    <input type="checkbox" name="isTopPopular" id="isTopPopular"
                        {{ $event->isTopPopular ? 'checked' : '' }}>
    ...
    <script>
        ...
        let layout = JSON.parse(JSON.parse(@json($layouts))); // Store the list of layouts
        renderList()
